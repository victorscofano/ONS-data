...
SELECT
        nom_subsistema,
        round(avg(val_gerhidraulica * 100.0 / NULLIF(totalGerado, 0)), 2) AS "participMedHidraulica(%)",
        round(avg(val_gertermica * 100.0 / NULLIF(totalGerado, 0)), 2) AS "participMedTermica(%)",
        round(avg(val_gereolica * 100.0 / NULLIF(totalGerado, 0)), 2) AS "participMedEolica(%)",
        round(avg(val_gersolar * 100.0 / NULLIF(totalGerado, 0)), 2) AS "participMedSolar(%)"
FROM (
    SELECT
            nom_subsistema,
            val_gerhidraulica,
